...
# We are only updating the time, not the date, so we need to extract the date from before
r = make_read_request('DateDelivery', 'cart', 'id = ' + str(CART_ID))
original_datetime = convertSQLDateTimeToDatetime(r.json()['rows'][0][0])

# Edit the original datetime with new time
new_datetime = original_datetime.replace(hour=18, minute=59, second=5)
